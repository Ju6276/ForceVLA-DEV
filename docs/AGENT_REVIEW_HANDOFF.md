...
3. **待定案**：force-blind 在主指标上被 force-sighted 稳定击败，主配置是否要改（见下）。
...
| **对 expert 平移增益（主指标）** | 0.078 ± 0.005 | **0.144 ± 0.031** | 0.090 ± 0.009 |
| 对 expert 旋转增益 | 0.038 ± 0.008 | **0.048 ± 0.004** | −0.140 ± 0.004 |
...
#### force-blind：**主指标上被击败，需要决策**

两个方向互相矛盾，且都不是噪声（三 seed 无重叠）：

- 力盲**赢**在对 teacher 的忠实度：陈旧头增益 +0.038 vs −0.095（力可见比直接输出零还差），
  合成平移 RMSE 0.00865 vs 0.01114（per-seed 0.0085/0.0086/0.0088 vs 0.0118/0.0113/0.0104）。
- 力可见**赢**在对 expert 的接近度，也就是我们自己定的主指标：0.144 vs 0.078
  （per-seed 0.138/0.110/0.185 vs 0.084/0.071/0.078），旋转也略胜。且只跑一遍 decoder，
  fast path 成本减半。

一种解释：力可见时陈旧头不再是陈旧修正器，而是第二个无约束的力条件头。它不再跟踪
teacher 的 null 漂移（陈旧增益转负、对 teacher 合成变差），但组合出的指令碰巧更贴近人类示教。
若如此，**分解叙事（力项 + 陈旧项）就不再由架构保证，只剩损失函数的约束**。

需要决策的是：主指标继续用"对 expert"，还是承认蒸馏方法应以"对 teacher"为主、
把对 expert 降为辅助指标。当前论文两处都在用，这次消融把这个不一致逼了出来。
在定下来之前，不要把「力盲是必要的」写进论文。
