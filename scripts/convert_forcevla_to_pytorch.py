...
import openpi.models.force_encoder
import openpi.models.model
import openpi.models_pytorch.pi0_pytorch
import openpi.models_pytorch.forcevla_pytorch
...
def slice_initial_orbax_checkpoint(checkpoint_dir: str, restore_precision: str | None = None):
    ...
    merged_llm, merged_paths = lora_merge.merge_openpi_gemma_lora(params["PaliGemma"]["llm"])
    params["PaliGemma"]["llm"] = merged_llm
    return {
        "paligemma_params": traversals.flatten_mapping(params["PaliGemma"], sep="/"),
        "projection_params": params,
        "merged_lora_paths": merged_paths,
    }
...
def convert_pi0_checkpoint(checkpoint_dir: str, precision: str, output_path: str, model_config):
    """
    ...
    force_encoder_config = openpi.models.force_encoder.ForceEncoderConfig(
        type="tcn", sampling_rate_hz=100, window_ms=100, max_sample_age_ms=12
    )
    pi0_model = openpi.models_pytorch.forcevla_pytorch.ForceVLAPytorch(model_config, force_encoder_config)
    ...
    allowed_missing = {
        name for name in pi0_model.state_dict() if name.startswith(("temporal_force_encoder.", "limoe."))
    }
    # These output embeddings are tied to the input embeddings by the HF model.
    ...
    if unexpected_missing or incompatible.unexpected_keys:
        raise ValueError(
            f"Unsafe core conversion: missing={sorted(unexpected_missing)}, "
            f"unexpected={sorted(incompatible.unexpected_keys)}"
        )
    torch_force_encoder.load_jax_params(
        pi0_model.temporal_force_encoder, initial_params["projection_params"]["temporal_force_encoder"]
    )
    torch_limoe.load_jax_params(pi0_model.limoe, initial_params["projection_params"]["limoe"])
    ...
    config_dict = {
        ...
        "precision": precision,
        "force_encoder_type": "tcn",
        "force_sampling_rate_hz": 100,
        "force_window_ms": 100,
        "merged_lora_paths": initial_params["merged_lora_paths"],
    }
...
def main(
    ...
    *,
    ...
    model_config = SimpleNamespace(
        pi05=False,
        # ForceVLA keeps OpenPI's 32D padded internal action/state width; the
        # Button data transform exposes the valid 10D robot command externally.
        action_dim=32,
        action_horizon=50,
